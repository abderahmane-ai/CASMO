...
def run_benchmark(
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"Epoch {epoch+1}/{num_epochs} | Eps: {epsilon:.2f} | Train: {train_acc:.1f}% | Val: {val_acc:.1f}% | Loss: {train_loss:.4f}"
        )
...
def main():
    ...
    plt.plot(
        adam_res["epsilons"], adam_res["val_accs"], label="DP-AdamW", color="orange", marker="x"
    )
    ...
    if casmo_res["confidence_values"]:
        ...
        plt.plot(
            steps,
            casmo_res["confidence_values"],
            color="green",
            alpha=0.6,
            label="CASMO Confidence",
        )

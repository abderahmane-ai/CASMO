...
def train_on_task(
    model, optimizer, train_loader, task_id, epochs=3, grad_accum_steps=32, results_dict=None
):
            ...
            if (i + 1) % grad_accum_steps == 0:
                ...
                if results_dict is not None and hasattr(optimizer, "group_metrics"):
                    ...
                    if agar is not None:
                        ...
                        results_dict["steps"].append(
                            global_step + (task_id * len(train_loader) // grad_accum_steps * epochs)
                        )
...
def run_continual_learning(
    ...
    if optimizer_name == "casmo":
        optimizer = CASMO(
            ...
            lr=lr,
            weight_decay=0.01,
            robustness=1.0,  # task conflict shows up as low-SNR gradients
            betas=(0.9, 0.999),
        )
    else:
    ...
    for task_id in range(4):
        test_dataset = ContinualLearningDataset(
            tokenizer, task_id, split="test", max_samples=100, max_length=256
        )
    ...
    for task_id in range(4):
        ...
        train_dataset = ContinualLearningDataset(
            tokenizer, task_id, split="train", max_samples=200, max_length=256
        )
        train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

        # Train on current task
        task_losses = train_on_task(
            model,
            optimizer,
            train_loader,
            task_id,
        ...
        for eval_task_id in range(task_id + 1):
            ...
            print(
                f"  Task {eval_task_id} ({task_names[eval_task_id]}): "
...
def plot_results(casmo_results, adamw_results, save_dir):
    ...
    for task_id in range(3):  # Exclude last task (no forgetting possible)
        ...
        if len(casmo_accs) > 1:
            ...
            ax.plot(
                ...
                casmo_forgetting,
                marker="o",
                label=f"CASMO: {task_names[task_id]}",
                linewidth=2,
            )
            ax.plot(
                x_points,
                adamw_forgetting,
                marker="s",
                label=f"AdamW: {task_names[task_id]}",
                linewidth=2,
                linestyle="--",
                alpha=0.7,
            )
    ...
    fig.suptitle("B4: Continual Learning - Sequential Task Performance", fontsize=16)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "plot1_forgetting_curves.png"), dpi=150, bbox_inches="tight")
    print("✅ Plot 1 saved to plot1_forgetting_curves.png")
    plt.close()
    ...
    print("✅ Plot 2 saved to plot2_final_performance.png")
    plt.close()
    ...
    if casmo_results.get("agar_history"):
        ...
        if total_steps > 0:
            ...
            ax1.text(
                total_steps - steps_per_task / 2, max(agar) * 0.9, "Task 3", ha="center", alpha=0.6
            )

        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, "plot3_mechanism.png"), dpi=150, bbox_inches="tight")
        print("✅ Plot 3 saved to plot3_mechanism.png")
        plt.close()
    ...
    print("✅ Plot 4 saved to plot4_summary_metrics.png")
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(description="B4: Continual Learning Benchmark")
    parser.add_argument(
        "--epochs_per_task", type=int, default=2, help="Epochs to train on each task"
    )
    parser.add_argument("--batch_size", type=int, default=1, help="Batch size")
    parser.add_argument(
        "--grad_accum_steps", type=int, default=32, help="Gradient accumulation steps"
    )
    ...
    print("=" * 70)
    print("B4: Continual Learning Benchmark")

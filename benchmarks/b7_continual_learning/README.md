...
## Ablation Study: High-Conflict Stress Test

To validate that AGAR correctly detects gradient conflicts, we conducted a controlled ablation study with an **extreme conflict scenario**.

### Setup
- **Task A**: Math problems in Format 1 (`Question: ... Answer: ...`)
- **Task B**: Same problems in Format 2 (`Input: ... Output: ...`)
- **Goal**: Force the model to learn contradictory formatting for identical content
...
![Confidence Histogram](ablation_results/mechanism_confidence_hist.png)
...
```bash
# Main sequential multi-task benchmark
python train.py

# Ablation: High-conflict stress test
python ablation_conflict_test.py
```

## Conclusion

CASMO successfully mitigates catastrophic forgetting in sequential fine-tuning. The ablation study validates that AGAR correctly detects gradient conflicts and adjusts learning rates automatically, making CASMO a robust choice for continual learning applications.
...
class LongTailCIFAR100(Dataset):
    ...
    def __init__(self, root="./data", train=True, imbalance_factor=100, seed=42):
        ...
        self.dataset = torchvision.datasets.CIFAR100(
            root=root, train=train, download=True, transform=None  # We'll apply transforms later
        )
...
def compute_per_group_accuracy(
    model, loader, many_shot_classes, medium_shot_classes, few_shot_classes, device
):
    ...
    many_acc = 100.0 * many_correct / many_total if many_total > 0 else 0
    medium_acc = 100.0 * medium_correct / medium_total if medium_total > 0 else 0
    few_acc = 100.0 * few_correct / few_total if few_total > 0 else 0
...
def run_benchmark(
    ...
    if optimizer_name == "casmo":
        optimizer = CASMO(
            ...
            robustness=1.0,  # rare classes give high-variance gradients
            c_min=0.1,
        )
    else:
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 20 == 0:
            print(
                f"Epoch {epoch+1}/{num_epochs} | Train: {train_acc:.1f}% | Val: {val_acc:.1f}% | "
    ...
    if optimizer_name == "casmo" and results["agar_values"]:
        print(f"\n{'='*70}")
        print("Final AGAR Statistics (last 100 steps):")
        print(f"{'='*70}")
        agar_final = (
            results["agar_values"][-100:]
            if len(results["agar_values"]) >= 100
            else results["agar_values"]
        )
        ...
        print(f"\n{'='*70}")
        print("Final Confidence Statistics (last 100 steps):")
        print(f"{'='*70}")
        conf_final = (
            results["confidence_values"][-100:]
            if len(results["confidence_values"]) >= 100
            else results["confidence_values"]
        )
...
def main():
    ...
    parser.add_argument(
        "--imbalance", type=int, default=100, help="Imbalance factor (e.g., 100 for 100:1 ratio)"
    )
    ...
    print("=" * 70)
    print("CASMO vs AdamW Benchmark - Long-Tailed CIFAR-100")
    ...
    axes[0, 0].plot(
        casmo_results["train_accs"], label="CASMO Train", linestyle="--", alpha=0.5, color="green"
    )
    axes[0, 0].plot(casmo_results["val_accs"], label="CASMO Val", linewidth=2, color="green")
    axes[0, 0].plot(
        adamw_results["train_accs"], label="AdamW Train", linestyle="--", alpha=0.5, color="orange"
    )
    ...
    axes[0, 1].plot(
        epochs,
        casmo_results["many_shot_accs"],
        label="CASMO Many-shot",
        color="green",
        linestyle="-",
        linewidth=2,
    )
    axes[0, 1].plot(
        epochs,
        casmo_results["medium_shot_accs"],
        label="CASMO Medium-shot",
        color="green",
        linestyle="--",
        linewidth=2,
    )
    axes[0, 1].plot(
        epochs,
        casmo_results["few_shot_accs"],
        label="CASMO Few-shot",
        color="green",
        linestyle=":",
        linewidth=2,
    )
    axes[0, 1].plot(
        epochs,
        adamw_results["many_shot_accs"],
        label="AdamW Many-shot",
        color="orange",
        linestyle="-",
        linewidth=2,
        alpha=0.7,
    ...
    axes[0, 1].plot(
        epochs,
        adamw_results["medium_shot_accs"],
        label="AdamW Medium-shot",
        color="orange",
        linestyle="--",
        linewidth=2,
        alpha=0.7,
    ...
    axes[0, 1].plot(
        epochs,
        adamw_results["few_shot_accs"],
        label="AdamW Few-shot",
        color="orange",
        linestyle=":",
        linewidth=2,
        alpha=0.7,
    ...
    if casmo_results["agar_values"]:
        axes[1, 0].plot(
            casmo_results["steps"], casmo_results["agar_values"], color="green", alpha=0.6
        )
    ...
    if casmo_results.get("confidence_values"):
        axes[1, 1].plot(
            casmo_results["steps"], casmo_results["confidence_values"], color="blue", alpha=0.6
        )
    ...
    print(f"{'-'*70}")
    print(
        f"{'Overall Val Accuracy':<30} {casmo_results['val_accs'][-1]:>14.2f}% {adamw_results['val_accs'][-1]:>14.2f}%"
    )
    print(
        f"{'Many-shot Accuracy':<30} {casmo_results['many_shot_accs'][-1]:>14.2f}% {adamw_results['many_shot_accs'][-1]:>14.2f}%"
    )
    print(
        f"{'Medium-shot Accuracy':<30} {casmo_results['medium_shot_accs'][-1]:>14.2f}% {adamw_results['medium_shot_accs'][-1]:>14.2f}%"
    )
    print(
        f"{'Few-shot Accuracy':<30} {casmo_results['few_shot_accs'][-1]:>14.2f}% {adamw_results['few_shot_accs'][-1]:>14.2f}%"
    )
    print(f"{'='*70}\n")

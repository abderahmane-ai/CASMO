"""
Benchmark B5: Multi-Asset Portfolio Optimization (DeepDow)
Benchmark ID: B5
        ...
        if HAS_YFINANCE:
            try:
                ...
                if len(available_tickers) != len(tickers):
                    print(
                        f"⚠️  Warning: Missing tickers. Request: {tickers}, Found: {available_tickers}"
                    )
...
class PortfolioAllocator(nn.Module):
    ...
    def __init__(self, num_assets, hidden_size=64):
        super().__init__()
        self.lstm = nn.LSTM(
            input_size=num_assets, hidden_size=hidden_size, num_layers=1, batch_first=True
        )
...
def main():
    parser = argparse.ArgumentParser(description="B5 Portfolio Optimization")
    ...
    print("\n" + "=" * 115)
    print(
        f"{'Strategy':<18} | {'CAGR':<8} | {'Sharpe':<6} | {'Sortino':<7} | {'MaxDD':<7} | {'Calmar':<7} | {'Turnover':<8}"
    )
    print("-" * 115)
    for m in metrics:
        print(
            f"{m['Name']:<18} | {m['CAGR']*100:>7.1f}% | {m['Sharpe']:>6.2f} | {m['Sortino']:>7.2f} | {m['MaxDD']*100:>6.1f}% | {m['Calmar']:>7.2f} | {m['Turnover']*100:>7.2f}%"
        )
    print("=" * 115)
    ...
    ax1.plot(
        dates, res_adam["equity"][1:], label="AdamW (Standard)", color="#e74c3c", linewidth=1.5
    )
    ax1.plot(
        dates, res_casmo["equity"][1:], label="CASMO (Proposed)", color="#2ecc71", linewidth=2.5
    )

    ax1.set_title("Cumulative Wealth (Test Phase: 2020-2023)", fontsize=14, fontweight="bold")
    ...
    ax2 = axes[1]
    ax2.stackplot(
        dates, res_casmo["weights"].T, labels=train_ds.tickers, alpha=0.85, cmap="viridis"
    )

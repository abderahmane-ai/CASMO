...
def run_benchmark(
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 50 == 0:
            print(
                f"Epoch {epoch+1}/{num_epochs} | Train Acc: {train_acc:.1f}% | Val Acc: {val_acc:.1f}% | Loss: {train_loss:.4f}"
            )
...
def main():
    ...
    axes[0, 0].axhline(
        y=expected_max_train,
        color="gray",
        linestyle=":",
        label=f"Max Clean Acc ({expected_max_train:.0f}%)",
    )
